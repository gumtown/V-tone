...
void GT100_data_change(int input_source, int value){
  if(device_mode == 1 || device_mode == 3){
  ...
  if(device_mode == 2 || device_mode == 4){
  if(input_source == 1) {knob20(1, 0, 0, value, LoGain, "60000132"); };
  if(input_source == 2) {mid_freq(1, 0, 1, value, HiMidFrq, "60000136"); };
  if(input_source == 3) {mid_q(2, 0, 0, value, HiMidQ, "60000137"); };
  if(input_source == 4) {knob20(2, 0, 1, value, HiMidGn, "60000138"); };
  if(input_source == 5) {knob20(3, 0, 0, value, HiGain, "60000139"); };
  //if(input_source == 6) {character(3, 0, 1, value, "Charactr", "061D"); };
  if(input_source == 7) {low_cut(1, 1, 0, value, "60000131"); };
  if(input_source == 8) {mid_freq(1, 1, 1, value, LoMidFrq, "60000133"); };
  if(input_source == 9) {mid_q(2, 1, 0, value, LoMidQ, "60000134"); };
  if(input_source == 10) {knob20(2, 1, 1, value, LoMidGn, "60000131"); };
  if(input_source == 11) {high_cut(3, 1, 0, value, "6000013A"); };
  if(input_source == 12) {knob20(3, 1, 10, value, Level, "6000013B"); };
  };
}

void GT100_amp_type(int value){
