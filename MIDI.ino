...
void prep_midi(String address, int value){
  String val=String(value, HEX); if(value<16){val=String("0")+val; };
  String message = address + val;
  String new_message;
  ...
  { 
    ...
    if(a>9){a=a-1;};                            // A~F seem to be off by 1 
    int b = String(message[x+1],16).toInt()-30; //convert char nibble to int
    if(b>9){b=b-1;};                            // A~F seem to be off by 1 (B~G)
    data_size = data_size + ((a*16)+b);
    a=a+48; if(a>57){a=a+7;};                   //ASCII table lookup - this is a retarded way to do it - but it works.
    b=b+48; if(b>57){b=b+7;};
    new_message.append(char(a)).append(char(b));    // rebuild message with missing A's
    ++x;
  };
  int sum = 128-(data_size % 128);
  if(sum==128) {sum=0; };
  String checksum = String(sum, 16); if(sum<16){checksum=String(("0")+checksum); }; 
  ...
  if(value != previous_value){ send_sysx(message); previous_value = value; };
...
void send_sysx(String message){
  ...
  for(int x=0; x<len; ++x)
  {
    ...
    if(a>9){a=a-1;};                           // A~F seem to be off by 1 
    int b = String(message[x+1],16).toInt()-30; //convert char nibble to int
    if(b>9){b=b-1;};                           // A~F seem to be off by 1 
    int c = (a*16)+b;
    array[x/2]= c;
    ++x;
  };
